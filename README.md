...
- `sido: "경기"`는 서버에서 `경기도`로 자동 정규화합니다. 가능하면 공식 시도명 또는 행정코드를 사용하세요.
- `search_auctions`는 후보 탐색용입니다. 묶음물건과 법원 검색색인 갱신 시점 때문에 가격 상한이 개별 물건에 엄격히 적용되지 않거나 목록 최저가가 이전 회차일 수 있습니다.
- 최종 가격·보증금·매각기일을 제시하기 전 후보마다 `get_auction_case`를 호출하세요.
- 사건 상세 응답의 `verification.currentSaleTerms`가 최신 공고에서 추출한 우선 확인값입니다. `calculatedDepositWon`은 최저가×보증금률 산정치이므로 실제 입찰 전 법원 원문을 다시 확인하세요.
...
서버는 첫 단독 검색에 큰 고정 지연을 추가하지 않습니다. 대신 같은 검색의 짧은 캐시,
동시에 들어온 동일 요청 합치기, 법원 코드표 장기 캐시, 실제 법원 요청 직렬화를 사용해
체감 속도를 유지하면서 불필요한 중복 호출을 줄입니다. 캐시는 Vercel 인스턴스 메모리에
저장되므로 콜드 스타트나 새 인스턴스에서는 다시 채워집니다.

기본값은 개인용 대화형 조회에 맞춰져 있으며 필요할 때만 환경변수로 조정하세요.

| 환경변수 | 기본값 | 설명 |
|---|---:|---|
| `COURT_AUCTION_MIN_INTERVAL_MS` | `500` | 서로 다른 실제 법원 요청의 최소 시작 간격. 첫 요청에는 대기 없음 |
| `COURT_AUCTION_SEARCH_TTL_MS` | `45000` | 동일 조건검색 캐시 시간 |
| `COURT_AUCTION_CASE_TTL_MS` | `30000` | 동일 사건조회 캐시 시간 |
| `COURT_AUCTION_NOTICE_TTL_MS` | `120000` | 공고 및 공고상세 캐시 시간 |
| `COURT_AUCTION_COURT_CODES_TTL_MS` | `43200000` | 법원 코드표 캐시 시간 |
| `COURT_AUCTION_CACHE_MAX_ENTRIES` | `200` | 인스턴스당 최대 캐시 항목 수 |

간격을 `0`으로 설정할 수는 있지만 법원 사이트 보호와 안정적인 개인 사용을 위해 기본값
유지를 권장합니다. 차단 응답을 받은 뒤 간격을 낮추거나 반복 재시도하지 마세요.
